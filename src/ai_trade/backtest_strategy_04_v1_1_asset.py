"""Run Strategy 04 v1.1 on any cached equity ETF with 15-minute/1-hour bars."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backtest Strategy 04 v1.1 on cached equity ETF data."
    )
    ...
    signal_result = candidate_signals_v1_1(
        fifteen, hours, execution_params, indicator_params
    )
    ...
    report = {
        ...
        "warning": (
            "Historical research only. Equity ETF cost assumptions are retained from "
            "the SPY test and must not be used for futures without adjustment."
        ),
    }
    ...
    print(f"Saved {args.symbol.upper()} Strategy 04 v1.1 backtest to {args.output}")
    return 0
