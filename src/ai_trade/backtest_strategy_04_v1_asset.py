"""Run the original Strategy 04 v1 on any cached equity ETF."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backtest original Strategy 04 v1 on cached equity ETF data."
    )
    ...
    report = {
        ...
        "change_from_v1": "None — original v1 baseline.",
        ...
        "warning": "Historical research only; equity ETF cost assumptions retained.",
    }
    (args.output / "backtest_report.json").write_text(
        json.dumps(report, indent=2) + "\n", encoding="utf-8"
    )
    print(json.dumps(report["results"], indent=2))
    print(f"Saved {args.symbol.upper()} Strategy 04 v1 backtest to {args.output}")
    return 0
