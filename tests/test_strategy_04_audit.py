...
def test_clean_trade_passes_every_check():
    results = audit_trade(_signal(), _trade(), "2021-08-03T13:00:00Z", TIMESTAMPS, 0.25, SLIPPAGE_BPS)
...
def test_atr_stamped_at_the_decision_timestamp_passes_causality():
    ...
    results = audit_trade(signal, _trade(), "2021-08-03T13:00:00Z", TIMESTAMPS, 0.25, SLIPPAGE_BPS)
...
def test_zone_qualified_after_trigger_fails_causality():
    results = audit_trade(_signal(), _trade(), "2021-08-03T14:45:00Z", TIMESTAMPS, 0.25, SLIPPAGE_BPS)
...
def test_zone_qualified_exactly_at_trigger_timestamp_passes_causality():
    signal = _signal()
    results = audit_trade(signal, _trade(), signal.trigger_timestamp, TIMESTAMPS, 0.25, SLIPPAGE_BPS)
    assert _result(results, "causality_zone").passed is True


def test_stop_buffer_must_be_five_percent_of_atr():
    results = audit_trade(_signal(stop_buffer=0.09), _trade(), "2021-08-03T13:00:00Z", TIMESTAMPS, 0.25, SLIPPAGE_BPS)
    assert _result(results, "stop_buffer").passed is False


def test_long_stop_sits_below_zone_lower_by_the_buffer():
    results = audit_trade(_signal(), _trade(stop_price=436.50), "2021-08-03T13:00:00Z", TIMESTAMPS, 0.25, SLIPPAGE_BPS)
...
def test_entry_must_be_the_next_fifteen_minute_bar():
    ...
    results = audit_trade(_signal(entry_timestamp="2021-08-03T14:45:00Z"), trade, "2021-08-03T13:00:00Z", TIMESTAMPS, 0.25, SLIPPAGE_BPS)
    assert _result(results, "entry_timing").passed is False


def test_target_must_be_one_r_from_entry():
    results = audit_trade(_signal(), _trade(target_price=440.0), "2021-08-03T13:00:00Z", TIMESTAMPS, 0.25, SLIPPAGE_BPS)
...
def test_entry_before_ten_thirty_new_york_fails_session():
    ...
    results = audit_trade(_signal(entry_timestamp="2021-08-03T14:15:00Z"), trade, "2021-08-03T13:00:00Z", TIMESTAMPS, 0.25, SLIPPAGE_BPS)
...
def test_entry_exactly_at_fifteen_hundred_new_york_fails_session():
    ...
    results = audit_trade(_signal(entry_timestamp="2021-08-03T19:00:00Z"), trade, "2021-08-03T13:00:00Z", TIMESTAMPS, 0.25, SLIPPAGE_BPS)
    assert _result(results, "session").passed is False
...
def test_friday_entry_fails_session():
    ...
    results = audit_trade(_signal(entry_timestamp="2021-08-06T14:30:00Z"), trade, "2021-08-03T13:00:00Z", TIMESTAMPS, 0.25, SLIPPAGE_BPS)
    assert _result(results, "session").passed is False


def test_target_exit_below_target_price_fails_outcome():
    results = audit_trade(_signal(), _trade(exit_price=438.0), "2021-08-03T13:00:00Z", TIMESTAMPS, 0.25, SLIPPAGE_BPS)
    assert _result(results, "outcome").passed is False
...
def test_demand_zone_with_short_side_fails_side_match():
    ...
    results = audit_trade(signal, _trade(side="short"), "2021-08-03T13:00:00Z", TIMESTAMPS, 0.25, SLIPPAGE_BPS)
